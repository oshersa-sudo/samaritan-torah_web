...
(function () {
  ...
  var POP = (window.POPULATION || []).slice().sort(function (a, b) { return a.year - b.year; });
  ...
  ['viewport', 'stage', 'ruler', 'lanes', 'rosters', 'bands', 'gridlines', 'rail', 'railTrack',
   ...
   'backSite', 'brandHome', 'langs', 'foot', 'muteBtn', 'bgm',
   'popChart', 'popAxis'].forEach(function (id) {
    el[id] = document.getElementById(id);
  });

  var ppy = 1.4;                          // פיקסלים לשנה
  var enabled = { samaritan: 1, israel: 1, torah: 1, world: 1, people: 0,
                  priests: 1, pm: 1, pres: 1, pop: 1 };
  var selected = null;
  var selectedPop = -1;
  ...
  /* סיומת סדר באנגלית; בעברית ובערבית התבנית שבמילון כבר נכונה */
  function ordinal(n) {
    if (window.I18N.lang !== 'en') return n;
    var t = n % 10, h = n % 100;
  ...
  /* בלי ריפוד בקצה, השנים האחרונות לעולם לא יגיעו אל נקודת ה-0 שבשמאל */
  function stageWidth() { return SPAN * ppy + el.viewport.clientWidth; }
  ...
  function refreshRosters() {
    ...
    if (changed) { drawRosters(); drawEvents(); drawPop(); }
  }
  ...
  /* ─────────────── עקומת האוכלוסייה ───────────────
   * הסקאלה לוגריתמית: הקהילה נעה בין 141 נפש למיליון, ארבעה סדרי גודל,
   * ובסקאלה לינארית כל התקופה שמן המאה ה-12 ואילך הייתה נמחקת לקו אפס.
   */

  var POP_MIN = 100, POP_MAX = 1000000;
  var POP_DECADES = [100, 1000, 10000, 100000, 1000000];

  var POP_PAD_TOP = 14, POP_PAD_BOT = 10;   /* שלא ייגעו הקצוות בגבול האזור */

  function popY(n, h) {
    var lo = Math.log(POP_MIN), hi = Math.log(POP_MAX);
    var t = (Math.log(clamp(n, POP_MIN, POP_MAX)) - lo) / (hi - lo);
    var usable = h - POP_PAD_TOP - POP_PAD_BOT;
    return POP_PAD_TOP + (1 - t) * usable;
  }
  ...
  function drawPopAxis() {
    var h = el.popChart.clientHeight;
    if (!h) return;
    var html = '<div class="pa-title">' + esc(T('popAxis')) + '</div>';
    POP_DECADES.forEach(function (d) {
      var top = el.popChart.offsetTop + popY(d, h);
      html += '<div class="pa-tick" style="top:' + top.toFixed(1) + 'px">' + popShort(d) + '</div>';
    });
    el.popAxis.innerHTML = html;
  }

  function drawPop() {
    if (!enabled.pop || !POP.length) {
      el.popChart.innerHTML = '';
      el.popAxis.hidden = true;
      return;
    }
    el.popAxis.hidden = false;
    /* הגרף תופס את אותו אזור אנכי שבו יושבים האירועים.
       הגובה מגיע מן ה-CSS (bottom:16px), ולכן אינו תלוי בקריאה של אלמנט אחר. */
    el.popChart.style.top = el.lanes.style.top || (RULER_H + 'px');

    var h = el.popChart.clientHeight;
    if (!h) return;                     /* עוד לפני הפריסה הראשונה */
    var w = el.stage.offsetWidth;
    var pts = POP.map(function (p) { return { p: p, x: xOf(p.year), y: popY(p.n, h) }; });

    var line = pts.map(function (q, i) {
      return (i ? 'L' : 'M') + q.x.toFixed(1) + ' ' + q.y.toFixed(1);
    }).join(' ');
    var area = line + ' L' + pts[pts.length - 1].x.toFixed(1) + ' ' + h + ' L' +
      pts[0].x.toFixed(1) + ' ' + h + ' Z';

    /* קטע שבין שני אומדנים מצויר מקווקו — אינו נתון מתועד */
    var segs = '';
    for (var i = 1; i < pts.length; i++) {
      var a = pts[i - 1], b = pts[i];
      var est = a.p.kind === 'estimate' || b.p.kind === 'estimate';
      segs += '<path class="pop-line' + (est ? ' est' : '') + '" d="M' + a.x.toFixed(1) + ' ' +
        a.y.toFixed(1) + ' L' + b.x.toFixed(1) + ' ' + b.y.toFixed(1) + '"/>';
    }

    var grid = '';
    POP_DECADES.forEach(function (d) {
      var y = popY(d, h);
      grid += '<line class="pop-grid" x1="0" y1="' + y.toFixed(1) + '" x2="' + w +
        '" y2="' + y.toFixed(1) + '"/>';
    });

    var dots = '', labs = '';
    var showLabels = ppy > 0.35;
    pts.forEach(function (q, i) {
      dots += '<circle class="pop-dot' + (q.p.kind === 'estimate' ? ' est' : '') +
        (selectedPop === i ? ' hi' : '') + '" data-i="' + i + '" cx="' + q.x.toFixed(1) +
        '" cy="' + q.y.toFixed(1) + '" r="4.5"><title>' +
        esc(C.gregLabel(q.p.year) + ' — ' + q.p.n.toLocaleString('en-US')) + '</title></circle>';
      if (showLabels) {
        labs += '<text class="pop-lab" x="' + (q.x + 7).toFixed(1) + '" y="' +
          (q.y - 7).toFixed(1) + '">' + esc(popShort(q.p.n)) + '</text>';
      }
    });

    el.popChart.innerHTML =
      '<svg width="' + w + '" height="' + h + '">' +
      '<defs><linearGradient id="popGrad" x1="0" y1="0" x2="0" y2="1">' +
      '<stop offset="0%" stop-color="#4fd1a5" stop-opacity=".26"/>' +
      '<stop offset="100%" stop-color="#4fd1a5" stop-opacity="0"/>' +
      '</linearGradient></defs>' +
      grid + '<path class="pop-area" d="' + area + '"/>' + segs + dots + labs +
      '</svg>';
    drawPopAxis();
  }
  ...
  function openPop(i) {
    ...
    selectedPop = i;
    selected = null;
    el.cardYear.textContent = (p.kind === 'estimate' ? T('approx') : '') + C.gregLabel(p.year);
    el.cardTitle.textContent = p.n.toLocaleString('en-US') + ' — ' + T('popTitle');
    el.cardEras.textContent = C.creationLabel(p.year) + ' · ' + C.canaanLabel(p.year) + ' · ' +
      T(p.kind === 'census' ? 'popCensus' : 'popEstimate');
    el.cardBody.textContent = L('pop', String(p.year), 'o', p.note);
  ...
  function render() {
    el.stage.style.width = stageWidth() + 'px';
    drawRuler();
    drawBands();
    drawRosters();
    drawEvents();
  ...
  function goToYear(year, where) {
    /* 4px בלבד, כדי שהשנה שחיפשו תופיע בדיוק בקריאת נקודת ה-0 */
    var offset = where === 'center' ? el.viewport.clientWidth / 2 : 4;
  ...
  function openEvent(id) {
    ...
    el.cardPriest.textContent = pr && !pr.gap
      ? T('inTheDaysOf') + T('ttl_priests') + ' ' + itemName(ROSTERS[0], pr) +
        (pr.n ? T('inDynasty').replace('{n}', ordinal(pr.n)) : '') : '';
  ...
  /* מסמן את האירוע הנבחר בלי לצייר מחדש את כל המסלולים */
  function highlightSelected() {
    el.lanes.querySelectorAll('.ev.hi').forEach(function (n) { n.classList.remove('hi'); });
    if (!selected) return;
    var n = el.lanes.querySelector('.ev[data-id="' + selected + '"]');
  ...
  var muted = false;
  try { muted = localStorage.getItem('timelineMuted') === '1'; } catch (e) { /* אין אחסון */ }
  el.bgm.volume = 0;        /* לעולם לא להתחיל בעוצמה מלאה */

  function syncMuteBtn() {
    el.muteBtn.textContent = muted ? '🔇' : '🔊';
    el.muteBtn.classList.toggle('muted', muted);
    el.muteBtn.classList.toggle('playing', playing && !muted);
    el.muteBtn.title = T(muted ? 'unmuteTitle' : 'muteTitle');
  }

  /* עולה בהדרגה, כדי שהמוסיקה לא תיפול לאוזן בבת אחת */
  var fadeTimer = 0;
  function fadeTo(target, done) {
    clearInterval(fadeTimer);
    fadeTimer = setInterval(function () {
      var d = target - el.bgm.volume;
      if (Math.abs(d) < 0.02) {
        el.bgm.volume = target;
        clearInterval(fadeTimer);
        if (done) done();
        return;
      }
      el.bgm.volume = clamp(el.bgm.volume + (d > 0 ? 0.02 : -0.02), 0, 1);
    }, 60);
  }

  var VOLUME = 0.18;                    /* מוסיקת רקע — שקטה */

  function startMusic() {
    if (muted) return;
    el.bgm.volume = 0;
    var p = el.bgm.play();
    if (p && p.catch) p.catch(function () { /* הדפדפן חסם — לא נורא */ });
    fadeTo(VOLUME);
  }

  function stopMusic() {
    fadeTo(0, function () { el.bgm.pause(); });
  }

  el.muteBtn.addEventListener('click', function () {
    muted = !muted;
    try { localStorage.setItem('timelineMuted', muted ? '1' : '0'); } catch (e) { /* אין אחסון */ }
    if (muted) { clearInterval(fadeTimer); el.bgm.pause(); el.bgm.volume = 0; }
  ...
  el.popChart.addEventListener('click', function (e) {
    var n = e.target.closest('.pop-dot');
    if (n) openPop(+n.dataset.i);
  });
  ...
  el.cardClose.addEventListener('click', function () {
    el.card.hidden = true; selected = null; selectedPop = -1; highlightSelected(); drawPop();
  });
  ...
  el.filters.addEventListener('change', function (e) {
    ...
    if (!chip) return;
    var t = chip.dataset.track || chip.dataset.roster || chip.dataset.chart;
  ...
  document.querySelectorAll('#filters .chip').forEach(function (c) {
    var t = c.dataset.track, r = c.dataset.roster, ch = c.dataset.chart;
    var label = t ? T('tr_' + t) : r ? T('rs_' + r) : T('popChip');
