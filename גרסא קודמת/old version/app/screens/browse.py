...
def _sam_markup(text):
    ...
    for p in parts:
        ...
        if _HEB_RE.fullmatch(p):
            out.append(f'[font=Samaritan]{p}[/font]')
        else:
            out.append(f'[font=Hebrew]{p}[/font]')
    return ''.join(out)
...
def _verse_text(raw_lines):
    ...
    # Process line-by-line so verse numbers always stay at the right of their line
    visual = rtl_lines(raw_lines)
    if use_sam:
        return _sam_markup(visual), True, False
...
class BrowseScreen(Screen):
    ...
    def _render_verses(self, chapter_id, book_name, portion_name, chapter_num, portion_id=None):
        ...
        if not verses:
            ...
            return
        use_eng = getattr(App.get_running_app(), '_english_mode', False)
        if use_eng:
        ...
        use_sam = getattr(App.get_running_app(), '_font_mode', 'Hebrew') == 'Samaritan'
        fsize = sp(14) if use_sam else sp(20)
