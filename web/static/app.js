...
const I18N = {
  he: {
    ...
    whatsnew_pages:[
      '<p><b>גרסה 3.6 — ספר בהדפסה, וסרגל ניווט שמפנה דרך לטקסט</b></p><ul><li>בדו־שיח ההדפסה נוספה הבחירה <b>מה להדפיס</b> — הפרק הנוכחי או <b>הפרשה כולה</b>; בהדפסה הרגילה זה גיליון לכל פרק, על כותרתו, פירושו ומילונו</li><li><b>פורמט ספר</b>: העמוד נחלק לשניים — חציו העליון התורה ותו לא, חציו התחתון הפירוש, וביניהם קו ורווח צר. אין כותרות ואין מספרי פסוקים, כי כל פירוש קורא לפסוק בשמו: שתי מילותיו הראשונות, שלוש נקודות, שתי האחרונות ומקף. שני החצאים אינם זרם אחד — פירוש ארוך ממשיך תחת הפרקים הבאים עד שהוא נגמר — והכל בכתב שומרוני ובכתיב חסר</li><li><b>סרגל ניווט חדש</b>: בתוך פרשה דיסקית ובה חץ בלבד, אל פרשה אחרת גלולה ובה שם הפרשה, ואל מחוץ לספר אותה גלולה בדיו — שהצעד הגדול לא ייראה כאחד הקטנים</li><li>הסרגל <b>צף מעל הדף</b> ואינו לוקח ממנו רצועה: במסכי הטקסט הוא נמוג לאחר רגע וכל נגיעה מחזירה אותו, והוא מפנה דרך לדף המתהפך וחוזר ברגע שהעלה נחת. נגיעה בו אינה מותירה סימן</li><li><b>נעץ</b> חדש בשורת סימני הפרק, לצד הסימנייה, מקבע על המסך את <b>שורת הנתיב</b> ואת <b>חלון ההשמעה</b>: נעוץ — הם נשארים בזמן הקריאה; משוחרר — הם מפנים את רצועתם לטקסט וחוזרים באותה נגיעה שמחזירה את סרגל הניווט. הבחירה נשמרת בין ביקורים</li><li>התריס העליון מתקפל עד הקצה — נשארת שורה אחת ובה שם האפליקציה, והידית מושכת אותו בחזרה כמקודם; וכפתורי הפסוקים הוקטנו</li><li>ובדף הספרים, בזמן שהעמוד בוער, הכתב מתחלף הלוך ושוב בין הפונט העברי לכתב השומרוני — העמוד אינו נמדד מחדש והאש אינה נערכת מחדש</li></ul>',
  ...
  en: {
    ...
    whatsnew_pages:[
      '<p><b>Version 3.6 — A book on paper, and a nav bar that gets out of the way</b></p><ul><li>The print dialog gains <b>what to print</b> — this chapter or <b>the whole parasha</b>; in the ordinary printout that is one sheet per chapter, with its heading, commentary and glossary</li><li><b>Book format</b>: the page divides in two — the Torah alone above, the commentary below, a rule and a thin space between them. No headings and no verse numbers, because each comment names its verse by quoting it: the two words it opens with, three dots, the two it closes on, and a dash. The halves are not one stream — a long comment runs on beneath the chapters that follow until it ends — and all of it is set in Samaritan script and defective spelling</li><li>A <b>new nav bar</b>: inside a parasha a disc carrying a chevron alone; into another parasha a pill carrying its name; and out of the book that same pill in ink, so the largest step never looks like one of the small ones</li><li>The bar <b>floats over the page</b> instead of taking a band of it: on text screens it fades out after a moment and any touch brings it back, and it steps aside for a turning page and returns as the leaf lands. Touching it leaves no mark</li><li>A new <b>pin</b>, among the chapter signs beside the bookmark, holds the <b>path line</b> and the <b>play window</b> on screen: pinned, they stay while the reader reads; released, they give their band back to the text and return on the same touch that calls back the nav bar. The choice is remembered between visits</li><li>The top bar now folds all the way, leaving one line carrying the name of the app and the handle that pulls it back down as before; and the verse buttons were made smaller</li><li>And on the bookshelf, while the page burns, the writing turns back and forth between the Hebrew face and the Samaritan one — the page is never re-measured and the fire is never re-laid</li></ul>',
  ...
  ar: {
    ...
    whatsnew_pages:[
      '<p><b>الإصدار 3.6 — كتابٌ في الطباعة، وشريط تنقّل يُفسح الطريق للنصّ</b></p><ul><li>في حوار الطباعة أُضيف خيار <b>ما الذي يُطبع</b> — هذا الأصحاح أو <b>المقطع كاملاً</b>؛ وفي الطباعة العاديّة صفحةٌ لكلّ أصحاح بعنوانه وتفسيره ومعجمه</li><li><b>تنسيق الكتاب</b>: تنقسم الصفحة نصفين — التوراة وحدها أعلى، والتفسير أسفل، وبينهما خطّ ومسافة رفيعة. لا عناوين ولا أرقام آيات، لأنّ كلّ تفسير يسمّي آيته باقتباسها: كلمتاها الأوليان، ثلاث نقاط، وكلمتاها الأخيرتان وشَرطة. والنصفان ليسا تيّاراً واحداً — فالتفسير الطويل يمضي تحت الأصحاحات التالية حتّى ينتهي — وكلّه بالخطّ السامري وبالكتابة الناقصة</li><li><b>شريط تنقّل جديد</b>: داخل المقطع قرصٌ يحمل سهماً فقط، وإلى مقطع آخر كبسولةٌ تحمل اسمه، وإلى خارج السفر الكبسولة نفسها بالحبر، كي لا تبدو الخطوة الكبرى كإحدى الصغيرات</li><li>الشريط <b>يطفو فوق الصفحة</b> ولا يقتطع منها شريطاً: في شاشات النصّ يتلاشى بعد لحظة وأيّ لمسة تعيده، ويفسح الطريق للصفحة المنقلبة ويعود ما إن تحطّ. ولمسه لا تترك أثراً</li><li><b>مِسمار</b> جديد بين علامات الأصحاح، إلى جانب علامة الحفظ، يثبّت على الشاشة <b>سطر المسار</b> و<b>نافذة التشغيل</b>: مثبّتين يبقيان أثناء القراءة، ومحرَّرين يعيدان شريطهما إلى النصّ ويعودان باللمسة نفسها التي تستدعي شريط التنقّل. ويُحفظ الاختيار بين الزيارات</li><li>والشريط العلويّ يُطوى حتّى الحافة — يبقى سطرٌ واحد فيه اسم التطبيق، والمقبض يعيده كما كان؛ وصغّرت أزرار الآيات</li><li>وفي رفّ الكتب، بينما تحترق الصفحة، تتبدّل الكتابة جيئةً وذهاباً بين الخطّ العبريّ والسامريّ — دون إعادة قياس الصفحة ودون إعادة إشعال النار</li></ul>',
...
function startPoemFire(){
  ...
  // the scraps fly over the whole window, so their canvas hangs from the body
  // itself, clear of anything that might clip it; it goes when the poem goes
  let wind = document.querySelector('.pscraps');
  if(!wind){
    wind = el('canvas','pscraps'); wind.setAttribute('aria-hidden','true');
  ...
  let wx = null;
  if(wind){
    wind.width = Math.round(innerWidth * sdpr); wind.height = Math.round(innerHeight * sdpr);
    wx = wind.getContext('2d');
    wx.setTransform(sdpr, 0, 0, sdpr, 0, 0);
  }
  ...
  // kept with it; that is roughly, but only roughly, the way the tongue goes.
  const seats = [];                     // index·4 + 0 left · 1 right · 2 up · 3 down
  for(let y = 0; y < H; y++)
    for(let x = 0; x < W; x++){
      const i = y*W + x;
      if(!fuel[i]) continue;
      if(x + 4 < W && !fuel[i+2] && !fuel[i+3] && !fuel[i+4]) seats.push(i*4 + 1);
      if(x - 4 >= 0 && !fuel[i-2] && !fuel[i-3] && !fuel[i-4]) seats.push(i*4);
      if(y - 4 >= 0 && !fuel[i-2*W] && !fuel[i-3*W] && !fuel[i-4*W]) seats.push(i*4 + 2);
  ...
  // its four margins — the second is simply a hole whose centre sits ON the edge,
  // wide along the edge and shallow into the page, so half of it falls outside
  // and what is seen is the margin eaten inward. Either way it opens, sometimes
  ...
  // side: 0 a hole in the page · 1 top · 2 bottom · 3 left · 4 right
  function openBurn(cx, cy, side){
    if(holes.length >= 3) return;
    const pts = [];
    for(let k = 0; k < 17; k++) pts.push(.62 + Math.random()*.66);
    const edge = !!side, big = edge ? 9 + Math.random()*11 : 7 + Math.random()*8;
    if(side === 1){ cx = 12 + Math.random()*(box.width - 24);  cy = 0; }
    if(side === 2){ cx = 12 + Math.random()*(box.width - 24);  cy = box.height; }
    if(side === 3){ cy = 12 + Math.random()*(box.height - 24); cx = 0; }
    if(side === 4){ cy = 12 + Math.random()*(box.height - 24); cx = box.width; }
    holes.push({
  ...
  function drawScars(){
    ...
    for(let i = holes.length - 1; i >= 0; i--){
      ...
      else if(a < lit){ r = h.rmax; ember = 1; fade = 1; }            // still alight
      else if(a < lit + h.glow){ r = h.rmax; ember = 1 - (a - lit)/h.glow*.92; fade = 1; }
      else {
  ...
  // A page that burns throws pieces of itself into the air. They leave the book
  // at its edge, tumble, are carried out toward the margins of the screen, and
  ...
  function drawScraps(){
    ...
    for(let i = scraps.length - 1; i >= 0; i--){
      const s = scraps[i], u = 1 - s.life / s.max;        // 0 whole → 1 wholly eaten
      s.x += s.vx + Math.sin(s.sway) * .5; s.y += s.vy;
      ...
      const at = k => { const a = k / n * Math.PI * 2, rr = r * s.pts[k % n];
                        return [Math.cos(a)*rr, Math.sin(a)*rr*.78]; };
      wx.save(); wx.translate(s.x, s.y); wx.rotate(s.rot); wx.globalAlpha = fade;
      wx.beginPath();
      for(let k = 0; k < n; k++){ const p = at(k); k ? wx.lineTo(p[0], p[1]) : wx.moveTo(p[0], p[1]); }
      wx.closePath();
      const g = wx.createLinearGradient(-r, -r, r, r);           // the paper itself
      g.addColorStop(0, '#f2ead6'); g.addColorStop(1, '#cdbe9c');
      wx.fillStyle = g; wx.fill();
      wx.strokeStyle = 'rgba(48,28,12,' + (.35 + u*.5) + ')';    // its charred edge
      wx.lineWidth = .8 + u*2.4; wx.stroke();
      // only part of the rim is alight, and the fire creeps round it as it flies
      s.edge += .05;
      wx.beginPath();
      const k0 = (s.edge / (Math.PI*2) * n) | 0;
      for(let j = 0; j <= 3; j++){ const p = at(k0 + j); j ? wx.lineTo(p[0], p[1]) : wx.moveTo(p[0], p[1]); }
      wx.strokeStyle = 'rgba(255,150,40,' + (.9 - u*.55) + ')';
      wx.lineWidth = 1.6; wx.lineCap = 'round';
      wx.shadowColor = 'rgba(255,110,20,.85)'; wx.shadowBlur = 5 + u*5;
  ...
  function step(){
    if(!alive || !cv.isConnected){
      alive = false;
      if(wind && wind.parentNode) wind.remove();     // the poem is gone; so is its ash
      return;
    }
    ...
    // out to every quarter, sideways, downward and up. Some of them curl: a slow
    // turning of the whole flame, one way and then the other, which draws it out
    ...
    for(const h of holes){
      ...
      if(flares.length < 4 && Math.random() < .045){
        ...
        flares.push({ x: gx, y: gy, vx: Math.cos(ang)*sp, vy: Math.sin(ang)*sp,
                      ...
                      lift: .03 + Math.random()*.035,
                      curl: Math.random() < .5, ph: Math.random()*6.28,
                      amp: .12 + Math.random()*.28, rate: .07 + Math.random()*.2 });
      }
    }
    for(let f = flares.length - 1; f >= 0; f--){
      ...
      if(fl.curl){                                       // the long lock, turning
        ...
        const nvx = fl.vx*c - fl.vy*s2, nvy = fl.vx*s2 + fl.vy*c;
        fl.vx = nvx; fl.vy = nvy;
      }
      fl.x += fl.vx; fl.y += fl.vy; fl.vy -= fl.lift; fl.vx *= .992;  // heat lifts it
      ...
      const onPage = fl.x > 6 && fl.x < W-7 && fl.y > 6 && fl.y < H-7;
      if(holeWait === 0 && inMargin && onPage && Math.random() < .5){
        holeWait = 260 + ((Math.random()*280)|0);        // nine seconds or more apart
    ...
    if(wind && (scraps.length || scrapDrawn)){ drawScraps(); scrapDrawn = scraps.length > 0; }
  }
  let scarDrawn = false, scrapDrawn = false;
  raf = requestAnimationFrame(step);
  _fireStop = () => {
...
// ── which chapters carry an animation, seen from the outside ────────────────
// Same rules as the blessings themselves; only the facts they judge are fetched,
// and the phrases the reading-cues watch are sent along so the rules never have
// to be written down a second time on the server.
const ANIM_ENDS   = ['כאשר צוה יהוה את משה'];
const ANIM_STARTS = ['ותכל כל עבדת המשכן'];
let _animMarks = {};
async function animMarks(bookId){
  if(_animMarks[bookId]) return _animMarks[bookId];
  const q = 'anim_marks?book_id=' + bookId
          + ANIM_ENDS.map(p => '&ends=' + encodeURIComponent(p)).join('')
          + ANIM_STARTS.map(p => '&starts=' + encodeURIComponent(p)).join('');
  try{ return (_animMarks[bookId] = await api(q)); }catch(e){ return {}; }
}
function chapterHasAnimation(marks, book, num, rows){
  const m = marks[String(num)];
  if(!m) return false;
  const first = _vfold(m.first || '');
  const prev = marks[String(num - 1)];
  const ctx = { book, num, prevWord: prev ? prev.last_word : null,
                opens: str => first.startsWith(_vfold(str)),
                has:   str => first.startsWith(_vfold(str)) };
  if(BLESSINGS.some(b => { try{ return b.when(ctx); }catch(e){ return false; } })) return true;
  if(book === 3 && LEV_REBUKE.some(o => first.startsWith(_vfold(o)))) return true;
  return !!(m.ends || (book === 2 && m.starts));
}
async function showSpread(){
  ...
  if(S.division==='samaritan'){
    ...
    // the administrator sees which chapters carry an animation. The facts the
    // rules need about a chapter seen from outside — its opening words, the word
    // the chapter before it ended on, and whether a verse inside it opens or
    // closes with the phrases the cues watch — are asked of the server once.
    const marks = ADMIN.token ? await animMarks(S.book) : null;
    for(const r of rows){
      const p = ch2port[r.id]||{id:null,name:''};
      const b=el('button','cell'+(marks && chapterHasAnimation(marks, S.book, r.number, rows) ? ' has-anim' : ''),
                 String(r.number));
      if(marks && chapterHasAnimation(marks, S.book, r.number, rows))
        b.appendChild(el('span','cell-anim','✦'));
...
// ── the animation flags, for the administrator only ─────────────────────────
// A lamp for "this chapter carries an animation", the words it says in a field
// of fifty, and two lamps for WHERE it fires: on entering the chapter, or on
// reaching its end. A blessing that answers a single verse lights neither, and
// says which verse it answers instead. All of it appears only while an admin is
// signed in — leaving admin mode repaints the chapter and it is gone.
function animFlag(label, on){
  return el('span','anim-flag'+(on?' on':''), '<i></i>' + esc(label));
}
...
function paintVerses(){
  ...
  if(ADMIN.token){
    ...
    c.appendChild(bar);
    if(S.chMode==='samaritan' && !S.english) c.appendChild(animAdminPanel());
  }
...
async function showTalFull(word){
  ...
  // The word's own meaning, quoted from a text that renders it. Shown FIRST and
  // never suppressed by a root: תרח resolves to the head-word Terah, while the
  // piyyutim use it 155 times for "gate" and Marqe translates it so.
  const mn=d&&d.meaning;
  if(mn){
    body.appendChild(el('div','tal-sec',t('mean_head')));
    const box=el('div','tal-infl');
    box.appendChild(el('div','tal-sense', esc(mn.gloss)));
    box.appendChild(el('div','note', t(mn.source==='memar'?'mean_memar':mn.source==='targum'?'mean_targum':'mean_torah')));
    if(mn.disagrees) box.appendChild(el('div','note','⚠ '+t('mean_clash')));
...
// The pin holds the chapter's own furniture: the path line at the top and the
// play window. Pressed, those two stay where they are while the reader reads;
// released, they keep out of the way and come back on the same touch that calls
// the nav bar back. The nav bar itself is not pinned — it floats over the page
// and takes no room from it, so there is nothing to give back and nothing to
// hold. The choice is the reader's, and is remembered between visits.
...
function armNavHide(){
  ...
  // off the text screens nothing hides at all. On them the class always goes on —
  // the nav bar always steps back — and the pin decides, in CSS, whether the path
